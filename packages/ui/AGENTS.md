...
**Chassi de UI do Mensvra Channel**: a folha de estilo do produto e os componentes de apoio que toda tela reusa. É uma **cópia adaptada** de `@mensvra-erp/ui` (o chassi do Mensvra ERP) — os dois produtos da família Mensvra compartilham a IDENTIDADE (paleta navy / azul elétrico / roxo, geometria, densidade), nunca o código vivo. Mesmo regime do `docs/08-reuso-ev-tracker.md`: cópia adaptada, nunca dependência. Plano: doc 12 §9.
...
2. **Nada de `@mensvra-erp/*`.** Import remanescente compila na máquina de quem copiou e quebra no CI de quem clonou. Preso pelo mesmo teste.
...
- **Nunca** trazer `escopo/` do chassi do ERP (seletor de empresa/filial). No Mensvra Channel o tenant vem SEMPRE da sessão JWT e nunca de escolha na interface (**regra inviolável 3**) — um seletor de empresa na tela é caminho para trocar de tenant pela UI, e não existe versão "só visual" disso que seja segura. Por tabela, não trazer `telas/`, `tabela/` e `consulta/`, que dependem dele.
...
- Nunca reintroduzir `formulario/` sem antes remover os campos fiscais (CNPJ, NCM, inscrição estadual): eles arrastam `@mensvra-erp/contracts`, que é domínio de ERP.
