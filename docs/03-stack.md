...
**Cartão de débito — exclusão consciente no MVP (divergência a validar).** O requisito original do produto pede crédito **e** débito, mas o suporte do Asaas a débito é limitado (apurado em 07/2026 na documentação oficial): a opção de débito só aparece no checkout da **fatura hospedada** do Asaas, em pagamento à vista (Visa/Mastercard, compensação D+3) — **não existe débito via API** ("enviando os dados do cartão pela API, infelizmente não", docs.asaas.com), nem em link de pagamento, nem em assinatura/recorrência. Decisão: débito **não é meio de pagamento de primeira classe no MVP** — quem abrir a fatura hospedada ganha a opção de débito à vista sem esforço nosso, mas o produto não promete, não integra nem exibe débito como método próprio. Racional: no Brasil pós-Pix, o pagamento à vista do público-alvo é Pix, que cobre o mesmo caso de uso com custo menor para o tenant (R$ 1,99 fixo vs. percentual de cartão) e liquidação imediata (vs. D+3). Porta de saída: a camada `PaymentProvider` permite driver futuro (Mercado Pago, já planejado como segundo driver) se débito transacional virar exigência. **Divergência com o requisito original — validar com o dono do projeto antes da Fase 2.**
