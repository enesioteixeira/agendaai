...
- [x] **Fase A — fundação visual e rebrand** (doc 12 §10): Tailwind 4 + chassi `@atende/ui` + tokens de marca Mensvra em `globals.css` (tema escuro padrão, sem piscar); shell do painel reescrito (`(painel)/layout.tsx`) com a marca **Mensvra Channel** e a navegação do produto — Atender / Vender / Automatizar / Configurar, itens não entregues visíveis e esmaecidos; `/inbox` esqueleto em três colunas lendo `Conversa`/`Mensagem` reais (lista viva; timeline e contexto são molduras da Fase B). A rota `/atendimento` continua no ar e é ela que abre ao clicar numa conversa — trocar a fundação visual não pode derrubar a inbox em produção
- [x] **E1 — inbox como operação** (`modules/inbox/`): a lista passou a ler `listarInbox` (`@atende/db`) em vez de montar o próprio `findMany` — traz fila, "não lidas" e a situação do prazo já calculada por `situacaoDoPrazo` do núcleo, sem N+1. Filtros por fila/estado/atendente/prazo vivem na URL (`?de=&estado=&fila=&prazo=`), são server-rendered com `<a href>` (funcionam sem JS, sobrevivem ao link compartilhado) e são lidos/validados em `modules/inbox/filtros.ts` — que descarta um `?fila=` que não é do tenant. Com filtro de prazo ligado, a ordenação passa a ser pelo prazo mais apertado (regra do `listarInbox`). Etiquetas aplicadas/removidas na barra da conversa com os tons do chassi; **nota interna** com endpoint próprio (`criarNotaInternaAction` → `NotaConversa`, que não sabe enviar), formulário separado do de resposta, moldura tracejada âmbar no campo e na timeline; respostas rápidas por `/atalho` no composer; **encerrar exige motivo** (`EncerrarConversa` → `encerrarComMotivoAction` → `encerrarConversa`), e a versão sem motivo foi REMOVIDA de `modules/atendimento/actions.ts` — `"use server"` publica endpoint, e mantê-la deixaria o caminho aberto. Polling segue sendo o pulso condicional de `modules/inbox/pulso.ts` (não trocar por outra estratégia)
