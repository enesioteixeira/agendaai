...
export function ConvidarForm({ papeis }: { papeis: { id: string; nome: string }[] }) {
  ...
  return (
    <div style={{ display: "grid", gap: "0.75rem" }}>
      ...
      {estado.linkConvite && (
        <div style={{ background: "var(--sucesso-fraco)", border: "1px solid var(--sucesso)", borderRadius: 8, padding: "0.75rem 1rem", fontSize: 14 }}>
          <strong>Convite criado para {estado.emailConvidado}</strong> (válido por 7 dias).
          <br />
          Envie este link para a pessoa:
          <div style={{ marginTop: 6, display: "flex", gap: 8, alignItems: "center" }}>
            <code style={{ background: "var(--superficie)", padding: "0.35rem 0.5rem", borderRadius: 6, border: "1px solid var(--borda)", wordBreak: "break-all", flex: 1 }}>
              {estado.linkConvite}
            </code>
            <button
              type="button"
          ...
          <p style={{ margin: "6px 0 0", color: "var(--texto-suave)", fontSize: 13 }}>
...
const ip: React.CSSProperties = { padding: "0.5rem 0.6rem", border: "1px solid var(--borda)", borderRadius: 6, fontSize: 15, minWidth: 220 };
const bt: React.CSSProperties = { padding: "0.55rem 0.9rem", background: "var(--acento)", color: "var(--acento-texto)", border: "none", borderRadius: 6, fontSize: 14, cursor: "pointer" };
