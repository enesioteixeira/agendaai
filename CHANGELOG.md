...
- **Mídia de conversa, ponta a ponta na entrada.** Imagem, áudio, vídeo e documento que o cliente manda passam a ser baixados, guardados e exibidos na timeline; antes o conector devolvia `midia: []` fixo e a bolha dizia "Mensagem de imagem". O download é entregue ao inbound como **função**, chamada só depois de o `create` passar pelo unique de dedupe — o WhatsApp reentrega, e baixar em cada reentrega seria rede e memória gastas para nada. O teto de 25 MB é conferido duas vezes: antes de baixar, pelo `fileLength` que o remetente declara, e depois, sobre os bytes que chegaram, porque o declarado é dele. A chave leva o tenant no prefixo, e a leitura sai por `/api/midia/[...chave]`, que confere o tenant **por segmento** — `startsWith` entregaria `emp_10` a quem tem `emp_1`. Só imagem, áudio e vídeo são renderizados: SVG e o resto descem como anexo, com `nosniff`, porque anexo renderizado na nossa origem é XSS armazenado.
- **O bloqueio da mídia era falso, e isso vale mais que a funcionalidade.** Desde o Bloco 0 os documentos diziam que a mídia dependia do binding R2 no `wrangler.jsonc` e da permissão correspondente no token da Cloudflare. Mas o R2 tem endpoint S3, e `packages/armazenamento` já era um driver S3 escrito com `fetch` e assinatura SigV4 à mão — que roda igual no Node do worker e no workerd do painel. O que faltava era credencial em variável de ambiente. O pacote estava pronto e órfão: nenhum app o importava. Registrado como divergência 14 no doc 11.
- **A imagem do worker passou a ser construível — ela nunca tinha sido.** O `Dockerfile` existia desde o Bloco 3 e nenhum `docker build` jamais o exercitou; a primeira tentativa morreu em `archive/tar: unknown file mode`, porque **não havia `.dockerignore`** e o contexto arrastava a floresta de symlinks do pnpm. Resolvido isso, a segunda morreu no `prisma generate`: o `Dockerfile` não copiava o **`.npmrc`**, que é onde mora `node-linker=hoisted` — sem ele o pnpm monta a árvore de symlinks padrão e o patch pós-generate não encontra o cliente do Prisma. As duas causas eram invisíveis enquanto ninguém construísse. Junto saiu o `|| pnpm install` de rede de segurança, que reescrevia o lockfile e transformava "o contexto de build está incompleto" numa imagem que sobe diferente do que foi testado.
- **Expirador de envio virou fila agendada.** `expirar-envios` estava declarada em `fila.ts` desde a Fase C, sem produtor e sem consumidor. É ela agora que fecha reserva órfã, a cada minuto, em vez de a varredura do outbox fazê-lo a cada 3 segundos — o teto é de dois minutos, e com dois workers o poller faria os dois varrerem as mesmas linhas.
